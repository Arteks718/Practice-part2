<template>
  <div id="app">
    <button @click="logOut" id="changeTheme">Вийти з акаунту</button>
    ...
    <table class="table table-dark">
      ...
      <tr v-for="(item, index) in students" v-bind:key="item._id">
        <td>
          <router-link v-bind:to="'/student-info/' + item._id" id="router-name">
            <span> {{ item.name }}</span>
          </router-link>
        </td>
        ...
        <td class="update">
          <i
            ...
            v-show="item.group === getCurrentUser.group"
          ></i>
        </td>
        <td class="delete">
          <a
